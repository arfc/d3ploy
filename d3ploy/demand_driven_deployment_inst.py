    ...
    def enter_notify(self):
        ...
        if self.fresh:
            ...
            for commod in self.commod_list:
                ...
                self.installed_capacity[commod][0] = 0.0
            ...
            self.buffer_dict = di.build_buffer_dict(
                self.supply_buffer, self.commod_list
            )
    ...
    def decision(self):
        ...
        for commod, proto_dict in self.commodity_dict.items():
            ...
            if self.record:
                ...
                out_text += " supply " + str(
                    self.commodity_supply[commod][time]
                )
                out_text += (
                    " demand "
    ...
    def calc_diff(self, commod, time):
        ...
        if self.buffer_type_dict[commod] == "perc":
            demand = self.predict_demand(commod, time) * (
                1 + self.buffer_dict[commod]
            )
        elif self.buffer_type_dict[commod] == "float":
            demand = (
                self.predict_demand(commod, time) + self.buffer_dict[commod]
            )
        else:
            raise Exception(
