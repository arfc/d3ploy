...
def plot_demand_supply(all_dict, commod, test, demand_driven, log_scale,calculated):
...
def plot_demand_supply_agent(all_dict, agent_dict, commod, test,
    ...
    if demand_driven:
        if log_scale:
            ax2.semilogy(*zip(*sorted(dict_demand.items())),'+',color='red',
                         label='Demand')
            if calculated:
                ax2.semilogy(*zip(*sorted(dict_calc_demand.items())),'o',alpha=0.5,color='red',
                            label='Calculated Demand')
        else:
            ax2.plot(*zip(*sorted(dict_demand.items())),'+',color='red',
                         label='Demand')
            if calculated:
                ax2.plot(*zip(*sorted(dict_calc_demand.items())),'o',alpha=0.5,color='red',
                            label='Calculated Demand')
    else:
        if log_scale:
            ax2.semilogy(*zip(*sorted(dict_demand.items())),'+',color='red',
    ...
    if log_scale:
        ax2.semilogy(*zip(*sorted(dict_supply.items())),'x',color='c',
                     label='Supply')
        if calculated:
            ax2.semilogy(*zip(*sorted(dict_calc_supply.items())),'o',color='c',
